...
def delete_group_message(message_id):
    if "user_id" not in session:
        flash("Você precisa estar logado para excluir uma mensagem.", "warning")
        return redirect("/login")
    ...
    if message.user_id != session["user_id"]:
        ...
        return redirect(f"/group/{message.group_id}")
    ...
    try:
        ...
        flash("Erro ao excluir a mensagem. Tente novamente.", "danger")
...
def delete_book():
    ...
    if book:
        # Excluir comentários associados ao jogo
        comments = Comment.query.filter_by(book_title=title).all()
...
def events():
    ...
    for event in events:
        print(url_for('delete_event', event_id=event.id))  # Isso ajudará a verificar a URL gerada
...
def delete_event(event_id):
    if "user_id" not in session:
        flash("Você precisa estar logado para excluir um evento.", "warning")
        return redirect("/login")
    
    event = Event.query.get_or_404(event_id)
    print(f"Evento encontrado: {event.name}")  # Para garantir que o evento está sendo encontrado
